...
async def generar_informe_mensual_auditado(context, user_id, mes_str, m, frecuencias=None, prompt_condicional=None):
    ...
    prom_act_ia = m.get('prom_minutos_act', 0)
    act_min_ia = m.get('act_min', 30)
    act_max_ia = m.get('act_max', 60)

    prompt_1 = (
        f"Actúa como un nutricionista clínico experto y empático. Contexto clínico del paciente:\n{condicion_clinica}\n"
        f"{contexto_harinas_str}\n"
        f"- Promedio de actividad física real del período: {prom_act_ia} min/día (Rango recomendado ajustado por impacto corporal: {act_min_ia} - {act_max_ia} min/día).\n"
        ...
        f"2. Céntrate exclusivamente en consejos de hábitos, interpretación de patrones de conducta alimentaria, constancia en la actividad física (valorando si realiza actividades de alto o bajo impacto) y orientaciones prácticas sobre cómo mejorar la calidad de las ingestas de forma sencilla.\n"
