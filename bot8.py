...
# Servidor Flask mínimo para que Render detecte el puerto abierto
app = Flask(__name__)
...
def analizar_imagen_con_groq(base64_image):
    ...
    response = client_ai.chat.completions.create(
        model="qwen/qwen2.6-vl-72b-instruct", # Asegurado modelo compatible con visión
        messages=[
            {
                "role": "user",
                "content": [
...
def main():
    ...
    # Inicia el servidor Flask en segundo plano para satisfacer el puerto de Render
    ...
    print("Bot iniciado correctamente en modo Polling.")
