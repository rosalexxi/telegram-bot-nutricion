...
GOOGLE_SHEETS_KEY_PATH = os.getenv(
    "GOOGLE_SHEETS_KEY_PATH", "credentials.json"
)
...
def calcular_tmb_y_get(
    peso, altura, edad, genero="masculino", actividad="sedentario"
):
...
def get_gspread_client():
    ...
    if os.path.exists(GOOGLE_SHEETS_KEY_PATH):
        creds = Credentials.from_service_account_file(
            GOOGLE_SHEETS_KEY_PATH, scopes=scopes
        )
    else:
...
def get_or_create_worksheet(spreadsheet, title):
    try:
        ...
        if title.startswith("User_"):
            ...
            ws = spreadsheet.add_worksheet(
                title=title, rows="500", cols="5"
            )
            ws.append_row(
                ["Fecha_Hora", "Fecha_Dia", "Alta", "Baja", "Pulsaciones"]
            )
            return ws
        elif title.startswith("Perfil_"):
            ws = spreadsheet.add_worksheet(
                title=title, rows="100", cols="7"
            )
            ws.append_row(
                [
                    "EDAD",
...
def obtener_datos_usuario(user_id):
    try:
        ...
        for c in df.columns:
            ...
            if "fecha" in c_lower:
                col_map[c] = "Fecha"
            elif "momento" in c_lower or "actividad" in c_lower:
                col_map[c] = "Momento"
            elif "alimento" in c_lower or "detalle" in c_lower:
                col_map[c] = "Alimento"
            elif "peso" in c_lower:
                col_map[c] = "Peso"
            elif "calor" in c_lower:
                col_map[c] = "Calorias"
            elif "prote" in c_lower:
                col_map[c] = "Proteinas"
            elif "grasa" in c_lower:
                col_map[c] = "Grasas"
            elif "hidrat" in c_lower or "carbo" in c_lower:
                col_map[c] = "Carbohidratos"
            elif "fibra" in c_lower:
                col_map[c] = "Fibras"
...
def analizar_imagen_con_groq(base64_image):
    if not client_ai:
        raise Exception("GROQ_API_KEY no está configurada correctamente.")
    prompt = (
        "Analizá esta imagen de comida/plato. Identificá los alimentos, estimá sus pesos en gramos y nutrientes. Respondé ÚNICAMENTE en formato JSON con la clave 'items' conteniendo alimento, peso, calorias, proteinas, grasas, carbohidratos, fibras."
    )
    response = client_ai.chat.completions.create(
...
def obtener_recomendacion_ia(resumen_texto):
    if not client_ai:
        return (
            "No se pudo obtener recomendación de IA (API Key no configurada)."
        )
    prompt = (
        f"Basado en este resumen de ingesta mensual y métricas del paciente, da una recomendación nutricional breve, profesional y motivadora (máximo 4 oraciones):\n\n{resumen_texto}"
    )
    try:
        response = client_ai.chat.completions.create(
            model="llama-3.3-70b-versatile",
        ...
        return (
            "Mantené una dieta equilibrada rica en fibra y agua, ajustando las porciones según tu actividad diaria."
        )
...
def generar_pdf_instrucciones_bytes():
    ...
    story = [
        Paragraph("MANUAL DE USO COMPLETO - BOT NUTRICIONAL", title_style),
        HRFlowable(
            width="100%",
            thickness=1.5,
        ...
        Paragraph(
            "• /start: Inicia el bot y reenvía este PDF informativo.",
            body_style,
        ),
        Paragraph(
            "• /comidas: Ver listado de comidas predeterminadas y plantilla en PDF.",
            body_style,
        ),
        Paragraph(
            "• /presion: Registrar mediciones de presión arterial (Ej: /presion 120,80,70).",
            body_style,
        ),
        Paragraph(
            "• /diario: Consultar consumos del día u otra fecha.", body_style
        ),
        Paragraph(
            "• /resumen: Obtener el resumen mensual con cálculo de TMB, presión y recomendaciones.",
            body_style,
        ),
        Paragraph(
        ...
        Paragraph("2. Entrada de Datos", section_style),
        Paragraph(
            "• Texto: Ingresá libremente tus alimentos o nombrá una plantilla predeterminada (Ej: *DESAYUNO).",
            body_style,
        ),
        Paragraph("• Voz: Grabá un audio describiendo lo consumido.", body_style),
        Paragraph(
            "• Foto: Enviá una foto de tu plato para análisis con inteligencia artificial.",
            body_style,
        ),
    ]
    doc.build(story)
...
def generar_pdf_comidas_bytes(plantillas):
    ...
    story = [
        Paragraph("LISTADO DE COMIDAS PREDETERMINADAS", title_style),
        HRFlowable(
            width="100%",
            thickness=1.5,
            color=colors.HexColor("#2563EB"),
    ...
    if not plantillas:
        story.append(
            Paragraph(
                "No hay comidas predeterminadas cargadas en la hoja 'Plantillas_Comidas'.",
                body_style,
            )
        )
    else:
        ...
        for p in plantillas:
            table_data.append(
                [
                    ...
                    Paragraph(
                        f"{p.get('Carbohidratos', 0):.1f}", body_style
                    ),
...
def generar_pdf_resumen_bytes(
    mes_str, df_mes, df_presion, perfil, tmb_val, recomendacion, user_id
):
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer,
    ...
    story = [
        Paragraph(f"Reporte Nutricional Mensual - {mes_str}", title_style),
        Paragraph(f"Usuario Telegram ID: {user_id}", body_style),
        HRFlowable(
            width="100%",
            thickness=1,
            color=colors.HexColor("#2563EB"),
    ...
    if not df_mes.empty:
        ...
        table_data_h1.append(
            [
                Paragraph("TOTAL MES", body_style),
                Paragraph(f"{tot_cons:.1f} kcal", body_style),
                Paragraph(f"{tot_quem:.1f} kcal", body_style),
                Paragraph(f"{tot_neto:.1f} kcal", body_style),
                Paragraph(f"{tot_prot:.1f} g", body_style),
                Paragraph(f"{tot_gras:.1f} g", body_style),
                Paragraph(f"{tot_carb:.1f} g", body_style),
                Paragraph(f"{tot_fibr:.1f} g", body_style),
            ]
        )
        div = dias_con_registro if dias_con_registro > 0 else 1
        table_data_h1.append(
            [
                Paragraph("PROM. DIARIO", body_style),
                Paragraph(f"{(tot_cons/div):.1f} kcal", body_style),
                Paragraph(f"{(tot_quem/div):.1f} kcal", body_style),
                Paragraph(f"{(tot_neto/div):.1f} kcal", body_style),
                Paragraph(f"{(tot_prot/div):.1f} g", body_style),
                Paragraph(f"{(tot_gras/div):.1f} g", body_style),
                Paragraph(f"{(tot_carb/div):.1f} g", body_style),
                Paragraph(f"{(tot_fibr/div):.1f} g", body_style),
            ]
        )
    t1 = Table(
        table_data_h1, colWidths=[65, 75, 70, 70, 65, 60, 80, 55]
    )
    t1.setStyle(
        TableStyle(
            [
    ...
    story.append(
        Paragraph("Análisis Metabólico y Estimación Corporal", title_style)
    )
    story.append(
        HRFlowable(
            width="100%",
    ...
    genero = (
        str(perfil.get("Sexo", perfil.get("Genero", "masculino")))
        if perfil
        else "masculino"
    )
    actividad = (
        str(perfil.get("Ocupacion", "sedentario"))
        if perfil
        else "sedentario"
    )
    ...
    table_bio = [
        [
            Paragraph("Dato Fisiológico", header_style),
            Paragraph("Valor Registrado", header_style),
        ],
        [Paragraph("Sexo", body_style), Paragraph(genero, body_style)],
        [
            Paragraph("Edad", body_style),
            Paragraph(f"{edad:.0f} años", body_style),
        ],
        [
            Paragraph("Peso", body_style),
            Paragraph(f"{peso:.1f} kg", body_style),
        ],
        [
            Paragraph("Altura", body_style),
            Paragraph(f"{altura:.1f} cm", body_style),
        ],
        [
            Paragraph("Ocupación / Actividad", body_style),
            Paragraph(actividad, body_style),
        ],
        [
            Paragraph("Metabolismo Basal (TMB)", body_style),
            Paragraph(f"{tmb:.1f} kcal / día", body_style),
        ],
        [
            Paragraph("Gasto Energético Conservador (GET)", body_style),
            Paragraph(f"{get_val:.1f} kcal / día", body_style),
        ],
    ]
    ...
    story.append(
        Paragraph("Resumen de Balance y Cambio Corporal Estimado:", sub_style)
    )
    ...
    table_comp = [
        [
            Paragraph("Métrica / Nutrientes", header_style),
            Paragraph("Promedio Diario Real (Hoja 1)", header_style),
            Paragraph("Estimado / Recomendado", header_style),
        ],
        [
            Paragraph("Calorías", body_style),
            Paragraph(f"{(tot_cons/dias_activos):.1f} kcal", body_style),
            Paragraph(f"{get_val:.1f} kcal (GET)", body_style),
        ],
        [
            Paragraph("Proteínas", body_style),
            Paragraph(f"{(tot_prot/dias_activos):.1f} g", body_style),
            Paragraph(f"{prot_rec:.1f} g", body_style),
        ],
        [
            Paragraph("Grasas", body_style),
            Paragraph(f"{(tot_gras/dias_activos):.1f} g", body_style),
            Paragraph(f"{gras_rec:.1f} g", body_style),
        ],
        [
            Paragraph("Carbohidratos", body_style),
            Paragraph(f"{(tot_carb/dias_activos):.1f} g", body_style),
            Paragraph(f"{carb_rec:.1f} g", body_style),
        ],
        [
            Paragraph("Fibras", body_style),
            Paragraph(f"{(tot_fibr/dias_activos):.1f} g", body_style),
            Paragraph(f"{fibr_rec:.1f} g", body_style),
        ],
    ]
    ...
    story.append(
        Paragraph(
            f"• BALANCE CALÓRICO NETO REAL: {bal_calorico:.1f} kcal",
            body_style,
        )
    )
    story.append(
        Paragraph(
            f"• CAMBIO ESTIMADO DE PESO: {cambio_peso_kg:.2f} kg ({cambio_peso_kg*1000:.1f} g)",
            body_style,
        )
    )
    story.append(Spacer(1, 12))
    story.append(Paragraph("Recomendación de la IA:", sub_style))
    story.append(Paragraph(f"{recomendacion}", body_style))
    doc.build(story)
    buffer.seek(0)
    return buffer
...
async def render_confirmation_screen(msg_or_query, context):
    ...
    for m in ["Desayuno", "Almuerzo", "Merienda", "Cena"]:
        ...
        m_buttons.append(
            InlineKeyboardButton(f"{mark}{m}", callback_data=f"set_m_{m}")
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"Item #{idx+1}", callback_data="noop"
                ),
                InlineKeyboardButton(
                    "🗑️ Anular", callback_data=f"del_item_{idx}"
                ),
            ]
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                f"{mark_hoy}Hoy", callback_data="set_d_hoy"
            ),
            InlineKeyboardButton(
                f"{mark_ayer}Ayer", callback_data="set_d_ayer"
            ),
            InlineKeyboardButton(
                f"{mark_otro}Otro Día", callback_data="set_d_otro"
            ),
        ]
    )
    keyboard.append(
        [
            InlineKeyboardButton(
                "🗑️ ELIMINAR TODO", callback_data="cancel_entry"
            ),
...
async def cmd_comidas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not plantillas:
        await update.message.reply_text(
            "📋 No hay comidas predeterminadas registradas en la hoja 'Plantillas_Comidas'."
        )
        return
        ...
        if len(parts) >= 3:
            try:
                ...
                mes = (
                    parts[5]
                    if len(parts) > 5
                    else obtener_ahora_arg().strftime("%Y-%m")
                )
                guardar_perfil_en_sheets(
                    user_id, edad, peso, altura, genero, ocupacion, mes
                )
                tmb, get_val = calcular_tmb_y_get(
                    peso, altura, edad, genero, ocupacion
                )
                await update.message.reply_text(
                ...
                await update.message.reply_text(
                    "❌ Error en los datos ingresados. Asegurate de usar números válidos."
                )
                return
...
async def cmd_presion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw_text:
        ...
        if len(parts) >= 3:
            guardar_presion_en_sheets(
                user_id, parts[0], parts[1], parts[2]
            )
            await update.message.reply_text(
                f"✅ **Presión registrada:**\nAlta: `{parts[0]}` | Baja: `{parts[1]}` | Pulsaciones: `{parts[2]}`",
                parse_mode="Markdown",
            )
            return
        else:
            await update.message.reply_text(
                "❌ Formato incorrecto. Uso: `/presion 120,80,70` o `/presion 12,75,65`",
                parse_mode="Markdown",
            )
            return
    await update.message.reply_text(
        "🩺 Ingresá la presión con el comando seguido de los valores. Ejemplo:\n`/presion 120,80,70`",
        parse_mode="Markdown",
    )
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("📆 Ayer", callback_data="diario_ayer"),
            ],
            [
                InlineKeyboardButton(
                    "🗓️ Seleccionar Fecha", callback_data="diario_otro"
                )
            ],
        ]
    )
...
async def cmd_resumen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "📅 Mes Actual", callback_data=f"resumen_mes_{mes_actual}"
                )
            ],
            [
                InlineKeyboardButton(
                    "📆 Mes Anterior",
                    callback_data=f"resumen_mes_{mes_anterior}",
                )
            ],
            [
                InlineKeyboardButton(
                    "🗓️ Otro Mes", callback_data="resumen_mes_otro"
                )
            ],
        ]
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if coincidencia:
        ...
        alimento_descripcion = (
            coincidencia.get("Detalle")
            or coincidencia.get("Alimento")
            or coincidencia.get("Descripcion")
            or coincidencia.get("Nombre")
        )
        item = {
            ...
            "carbohidratos": parse_raw_val(
                coincidencia.get("Carbohidratos")
            ),
        ...
        msg = await update.message.reply_text(
            "🔍 Plantilla localizada con éxito..."
        )
        await render_confirmation_screen(msg, context)
        return
    if raw_text.startswith("*"):
        await update.message.reply_text(
            f"❌ La plantilla `{raw_text}` no fue encontrada en Excel. Registro descartado.",
            parse_mode="Markdown",
        )
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "noop":
        ...
        context.user_data["pending_fecha"] = obtener_ahora_arg().strftime(
            "%Y-%m-%d"
        )
        await render_confirmation_screen(query, context)
    elif data == "set_d_ayer":
        context.user_data["pending_fecha"] = (
            obtener_ahora_arg() - timedelta(days=1)
        ).strftime("%Y-%m-%d")
        await render_confirmation_screen(query, context)
    elif data == "set_d_otro":
        await query.edit_message_text(
            "📅 Por favor, escribí la fecha deseada en formato `YYYY-MM-DD` (Ej: `2026-08-01`):",
            parse_mode="Markdown",
        )
        ...
        fecha = (obtener_ahora_arg() - timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
        await mostrar_diario_fecha(query, user_id, fecha)
    elif data == "diario_otro":
        await query.edit_message_text(
            "📆 Por favor enviá la fecha que querés consultar en formato `YYYY-MM-DD`:",
            parse_mode="Markdown",
        )
        context.user_data["awaiting_diario_date"] = True
    elif data == "resumen_mes_otro":
        await query.edit_message_text(
            "🗓️ Por favor enviá el mes que querés consultar en formato `YYYY-MM` (Ej: `2026-07`):",
            parse_mode="Markdown",
        )
...
async def mostrar_diario_fecha(query_or_update, user_id, fecha_str):
    ...
    if df.empty:
        ...
        if df_filtrado.empty:
            ...
            c_cons = df_filtrado[df_filtrado["Calorias"] > 0][
                "Calorias"
            ].sum()
            c_quem = abs(
                df_filtrado[df_filtrado["Calorias"] < 0]["Calorias"].sum()
            )
            b_neto = c_cons - c_quem
...
async def mostrar_resumen_mes(query_or_update, user_id, mes_str):
    ...
    if perfil:
        ...
        tmb_val, get_val = calcular_tmb_y_get(
            peso, altura, edad, genero, actividad
        )
    ...
    if not df_presion.empty:
        df_p_mes = (
            df_presion[df_presion["Fecha_Dia"].str.startswith(mes_str)]
            if "Fecha_Dia" in df_presion.columns
            else pd.DataFrame()
        )
        ...
        [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"descargar_pdf_resumen_{mes_str}",
                )
            ]
        ]
    )
    if hasattr(query_or_update, "edit_message_text"):
        await query_or_update.edit_message_text(
            txt, reply_markup=keyboard, parse_mode="Markdown"
        )
    else:
        await query_or_update.message.reply_text(
            txt, reply_markup=keyboard, parse_mode="Markdown"
...
async def generar_y_enviar_pdf_resumen(query, user_id, mes_str, context):
    ...
    df_presion = obtener_datos_presion(user_id)
    df_mes = (
        df[df["Fecha"].str.startswith(mes_str)]
        if not df.empty
        else pd.DataFrame()
    )
    tmb_val = 0
    ...
    rec_ia = obtener_recomendacion_ia(
        f"Resumen del mes {mes_str} para usuario {user_id}"
    )
