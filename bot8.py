...
async def generar_informe_mensual_auditado(context, user_id, mes_str, m, frecuencias=None, prompt_condicional=None):
    ...
    for intento_actual in range(1, max_intentos + 1):
        try:
            ...
            # 🌍 Títulos traducidos dinámicamente según el idioma del usuario
            titulo_1 = obtener_texto_db("04-titulo_diag_global", user_id)
            titulo_2 = obtener_texto_db("04-titulo_alimentos_inc", user_id)
            titulo_3 = obtener_texto_db("04-titulo_alimentos_red", user_id)

            informe_candidato = (
                f"<b>{titulo_1}</b><br/>{texto_p1}<br/><br/>"
                f"<b>{titulo_2}</b><br/>{texto_p2}<br/><br/>"
                f"<b>{titulo_3}</b><br/>{texto_p3}"
            )
...
async def render_confirmation_screen(msg_or_query, context):
    ...
    if momento != 'Actividad':
        ...
        for m in ["Desayuno", "Almuerzo", "Merienda", "Cena"]:
            ...
            # 🌍 Traducimos dinámicamente el momento según el idioma del usuario
            nombre_momento = obtener_texto_db(f"05-btn_mom_{m.lower()}", user_id)
            m_buttons.append(InlineKeyboardButton(f"{mark}{nombre_momento}", callback_data=f"set_m_{m}"))
    ...
    if not es_plantilla:
        for idx, item in enumerate(items, start=1):
            ...
            # 🌍 Traducimos "Editar" y "Anular" usando Supabase
            btn_editar_txt = obtener_texto_db("05-btn_editar", user_id)
            btn_anular_txt = obtener_texto_db("05-btn_anular", user_id)
            
            keyboard.append([
                InlineKeyboardButton(f"#{idx} {nombre_corto}", callback_data=f"noop_{idx}"),
                InlineKeyboardButton(f"✏️ {btn_editar_txt}", callback_data=f"edit_item_{idx}"),
                InlineKeyboardButton(f"❌ {btn_anular_txt}", callback_data=f"del_item_{idx}")
            ])
